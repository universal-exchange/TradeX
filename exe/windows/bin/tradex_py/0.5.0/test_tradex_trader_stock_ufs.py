...
class deploy_stock_ufs(object):
    def __init__(self):
        ...
        self.port = 8011
        self.trader = "tradex_trader_stock_ufs"
...
def HandleReturnData(result):
    try:
        ...
        if form == msg_code_json:
            ...
            elif func == func_stock_q_user_capital:
                print("资金应答:")
                if len(data) > 0:
                    for capital in data:
                        print("account:", capital["account"], "currency:", capital["currency"], "available:", capital["available"], "balance:", capital["balance"])
                else:
                    print("无资金记录。")
            elif func == func_stock_q_user_position:
                print("持仓应答:")
                if len(data) > 0:
                    for position in data:
                        print("holder:", position["holder"], "exchange:", position["exchange"], "currency:", position["currency"], "symbol:", position["symbol"], 
...
def QueryCapital(module, query, callback):
    ...
    g_caller_execute_success = False
    g_caller_dict[g_caller_id] = caller_item(g_caller_id)
    result = json.loads(module.DirectCall(g_caller_id, func_stock_q_user_capital, 0, query.ToJson(), callback)) # 异步
    print(result["return_code"], result["return_info"], result["caller_id"])
    if result["return_code"] != 0:
        return False
    else:
...
def QueryPosition(module, query, callback):
    ...
    g_caller_execute_success = False
    g_caller_dict[g_caller_id] = caller_item(g_caller_id)
    result = json.loads(module.DirectCall(g_caller_id, func_stock_q_user_position, 0, query.ToJson(), callback)) # 异步
    print(result["return_code"], result["return_info"], result["caller_id"])
    if result["return_code"] != 0:
        return False
    else:
...
def Test_TradeX_Trader_Stock_UFS():
    global g_order
    global g_session
    global g_strategy
    global g_caller_wait_time
    ...
    module = cyberx.Create("tradex_trader_stock_ufs") # 全局唯一
    #module_01 = cyberx.Create("tradex_trader_stock_ufs") # 重复创建会报异常
    #module_01 = cyberx.GetCreate("tradex_trader_stock_ufs") # 可以获取已创建的实例
    ...
    if False:
        g_query = query_item(g_strategy, g_session, 0, "", "", def_trade_task_query_type_easy)
