...
func_future_t_single_finish = 2200 # 单个期货委托成交 # 仅供测试交易
...
class deploy_test_fue(object):
    def __init__(self):
        self.flag = "test_fue"
        self.addr = "10.0.7.200"
        self.port = 8021
        self.trader = "tradex_trader_test_fue"
...
def FinishOrder(module, order, callback):
    ...
    g_caller_execute_success = False
    g_caller_dict[g_caller_id] = caller_item(g_caller_id)
    result = json.loads(module.DirectCall(g_caller_id, func_future_t_single_finish, 0, order.ToJson(), callback)) # 异步
    print(result["return_code"], result["return_info"], result["caller_id"])
    if result["return_code"] != 0:
        return False
    else:
...
def Test_TradeX_Trader_Test_FUE():
    global g_order
    global g_session
    global g_strategy
    global g_caller_wait_time
    ...
    module = cyberx.Create("tradex_trader_test_fue") # 全局唯一
    #module_01 = cyberx.Create("tradex_trader_test_fue") # 重复创建会报异常
    #module_01 = cyberx.GetCreate("tradex_trader_test_fue") # 可以获取已创建的实例
    
    subscribe_id_01 = module.SubscribeInfo(OnReturnInfo_01) # 订阅信息
    #subscribe_id_02 = module.SubscribeInfo(OnReturnInfo_02) # 订阅信息
    
    deploy = deploy_test_fue()
