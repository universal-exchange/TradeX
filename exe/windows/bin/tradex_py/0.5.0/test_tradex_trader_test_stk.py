...
func_stock_o_user_login    = 1101 # 期货用户登录
func_stock_o_user_logout   = 1102 # 期货用户登出
func_stock_t_single_finish = 1200 # 单个证券委托成交 # 仅供测试交易
func_stock_t_single_order  = 1201 # 单个证券委托下单
func_stock_t_single_cancel = 1202 # 单个证券委托撤单
func_stock_q_user_capital  = 1302 # 查询客户资金
func_stock_q_user_position = 1304 # 查询客户持仓
func_stock_r_cancel        = 1900 # 撤单回报
func_stock_r_order         = 1901 # 报单回报
func_stock_r_trans         = 1902 # 成交回报
...
class deploy_test_stk(object):
    def __init__(self):
        self.flag = "test_stk"
        self.addr = "10.0.7.200"
        self.port = 8011
        self.trader = "tradex_trader_test_stk"
...
class order_item(object):
    def __init__(self, strategy, session, symbol, exchange, entr_type, exch_side, price, amount, order_flag = 0):
        self.order_id = 0 # 委托编号
        self.batch_id = 0 # 批量委托编号
        self.symbol = symbol # 证券代码
        self.exchange = exchange # 交易所，SH:上交所，SZ:深交所
        self.entr_type = entr_type # 委托方式，A股，1:限价，2:市价，H股，1:竞价限价盘，2:增强限价盘，3:零股买卖
        self.exch_side = exch_side # 交易类型，A股，1:买入，2:卖出，29:申购，30:赎回，37:质押入库，38:质押出库，H股，1:买入，2:卖出
        self.price = price # 委托价格
        self.amount = amount # 委托数量 # 恒生UFS柜台申赎数量单位为篮子数即多少个最小申赎单位
        self.finish_price = 0.0 # 完成价格 # 仅供测试交易
        self.finish_amount = 0 # 完成数量 # 仅供测试交易
        self.fill_qty = 0 # 成交数量
        self.cxl_qty = 0 # 撤单数量
        # 0：未申报，1：正在申报，2：已申报未成交，3：非法委托，4：撤单失败/申请资金授权中，5：部分成交，6：全部成交，7：部成部撤，8：全部撤单，9：撤单未成，10：等待撤单
        self.status = 0 # 申报结果
        self.status_msg = "" # 申报说明 # 中文
...
class query_item(object):
    def __init__(self, strategy, session, order_id, symbol, exchange, query_type):
    ...
    try:
        ...
        if form == msg_code_json:
            ...
            if func == func_stock_r_order:
                print("报单回报:", data["order_id"], data["symbol"], data["exchange"], data["exch_side"], 
                                  data["fill_qty"], data["status"], data["status_msg"], data["session"], data["strategy"])
            elif func == func_stock_r_cancel:
                print("撤单回报:", data["order_id"], data["symbol"], data["exchange"], data["exch_side"], 
                                  data["fill_qty"], data["status"], data["status_msg"], data["session"], data["strategy"])
            elif func == func_stock_r_trans:
                print("成交回报:", data["order_id"], data["trans_id"], data["symbol"], data["exchange"], 
                                  data["exch_side"], data["fill_qty"], data["fill_price"], data["fill_time"], data["session"], data["strategy"])
            elif func == func_stock_t_single_order:
                print("报单应答:", data["status"], data["finish"], data["message"], data["session"], data["strategy"])
            elif func == func_stock_t_single_cancel:
    ...
    try:
        ...
        if caller_id in g_caller_dict.keys():
            ...
            if caller_item.return_code != 0:
                ...
                g_caller_execute_success = True
                caller_item.SetResultData(result)
                print("FinishOrder:", caller_item.return_info, caller_item.result_data)
        else:
            print("OnFinishOrder 调用编号 缺失！%d" % caller_id)
    except Exception as e:
        print("OnFinishOrder 异常！%s" % e)
    g_event_call_finish.set() #
...
def TraderStart(module, config, callback):
    ...
    result = json.loads(module.DirectCall(g_caller_id, func_stock_o_user_login, 0, config.ToJson(), callback)) # 异步
...
def TraderStop(module, config, callback):
    ...
    g_caller_execute_success = False
    g_caller_dict[g_caller_id] = caller_item(g_caller_id)
    result = json.loads(module.DirectCall(g_caller_id, func_stock_o_user_logout, 0, config.ToJson(), callback)) # 异步
    print(result["return_code"], result["return_info"], result["caller_id"])
    if result["return_code"] != 0:
        return False
    else:
...
def PlaceOrder(module, order, callback):
    ...
    g_caller_execute_success = False
    g_caller_dict[g_caller_id] = caller_item(g_caller_id)
    result = json.loads(module.DirectCall(g_caller_id, func_stock_t_single_order, 0, order.ToJson(), callback)) # 异步
    print(result["return_code"], result["return_info"], result["caller_id"])
    if result["return_code"] != 0:
        return False
    else:
...
def CancelOrder(module, order, callback):
    ...
    g_caller_execute_success = False
    g_caller_dict[g_caller_id] = caller_item(g_caller_id)
    result = json.loads(module.DirectCall(g_caller_id, func_stock_t_single_cancel, 0, order.ToJson(), callback)) # 异步
    print(result["return_code"], result["return_info"], result["caller_id"])
    if result["return_code"] != 0:
        return False
    else:
...
def FinishOrder(module, order, callback):
    global g_caller_id
    global g_caller_dict
    global g_caller_wait_time
    global g_caller_execute_success
    ...
    g_caller_execute_success = False
    g_caller_dict[g_caller_id] = caller_item(g_caller_id)
    result = json.loads(module.DirectCall(g_caller_id, func_stock_t_single_finish, 0, order.ToJson(), callback)) # 异步
    print(result["return_code"], result["return_info"], result["caller_id"])
    if result["return_code"] != 0:
        return False
    else:
        ...
        ret_wait = g_event_call_finish.wait(timeout = g_caller_wait_time) # 等待调用结果
        if ret_wait != True:
            print("等待 委托成交 结果超时！", caller_id)
            return False
        if g_caller_execute_success == False:
            print("委托成交 失败！", caller_id)
            return False
        else:
            print("委托成交 成功。", caller_id)
            return True
    return True

def Test_TradeX_Trader_Test_STK():
    global g_order
    global g_session
    global g_strategy
    global g_caller_wait_time
    ...
    module = cyberx.Create("tradex_trader_test_stk") # 全局唯一
    #module_01 = cyberx.Create("tradex_trader_test_stk") # 重复创建会报异常
    #module_01 = cyberx.GetCreate("tradex_trader_test_stk") # 可以获取已创建的实例
    
    subscribe_id_01 = module.SubscribeInfo(OnReturnInfo_01) # 订阅信息
    #subscribe_id_02 = module.SubscribeInfo(OnReturnInfo_02) # 订阅信息
    
    deploy = deploy_test_stk()
    ...
    if False:
        g_order = order_item(g_strategy, g_session, "600000", "SH", def_trade_entr_type_l, def_trade_exch_side_b, 400.0, 100)
    ...
    if True:
        ...
        g_order.finish_price = 4620.0
        g_order.finish_amount = 1
        result = FinishOrder(module, g_order, OnFinishOrder)
        print(result)
