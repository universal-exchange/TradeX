...
// 示例说明：
// 1、演示 tradex_trader_test_fue 交易接口插件的使用；
...
let func_future_t_single_finish = 2200 // 单个期货委托成交 // 仅供测试交易
...
class order_item {
    constructor(strategy, session, instrument, exchange, entr_type, exch_side, offset, hedge, price, amount, order_flag = 0) {
        ...
        this.finish_price = 0.0 // 完成价格 // 仅供测试交易
        this.finish_amount = 0 // 完成数量 // 仅供测试交易
    ...
    try {
        ...
        if(g_caller_dict.has(caller_id)) {
            ...
            if(caller_item.return_code !== 0) {
                ...
                g_caller_execute_success = true
                caller_item.SetResultData(result)
                console.log('FinishOrder:', caller_item.return_info, caller_item.result_data)
            }
        }
        else {
            console.log('OnFinishOrder 调用编号 缺失！' + caller_id)
        }
    }
    catch(error) {
        console.log('OnFinishOrder 异常！' + error)
    }
    g_event_call_finish.abort() //
}
...
async function FinishOrder(module, order, callback) {
    g_caller_id += 1
    g_caller_execute_success = false
    g_event_call_finish = new AbortController()
    g_caller_dict.set(g_caller_id, new caller_item(g_caller_id))
    let result = JSON.parse(module.DirectCall(g_caller_id, func_future_t_single_finish, 0, order.ToJson(), callback)) // 异步
    console.log(result['return_code'], result['return_info'], result['caller_id'])
    if(result['return_code'] !== 0) {
        return false
    }
        ...
        const ret_wait = await promises.setTimeout(g_caller_wait_time * 1000, '', { signal:g_event_call_finish.signal }).then(() => false, err => true) // 等待调用结果
        if(ret_wait != true) {
            console.log('等待 委托成交 结果超时！' + caller_id)
            return false
        }
        if(g_caller_execute_success === false) {
            console.log('委托成交 失败！' + caller_id)
            return false
        }
        else {
            console.log('委托成交 成功。' + caller_id)
            return true
        }
    }
    return true
...
async function Test_TradeX_Trader_Test_FUE() {
    let kernel = new cyberx.Kernel(new syscfg.SysCfg()) // 全局唯一
    let module = new cyberx.Create('tradex_trader_test_fue') // 全局唯一
    //let module_01 = new cyberx.Create('tradex_trader_test_fue') // 重复创建会报异常
    //let module_01 = new cyberx.GetCreate('tradex_trader_test_fue') // 可以获取已创建的实例
    ...
    let deploy = new deploy_test_fue()
    ...
    if(true) {
        ...
        result = await FinishOrder(module, g_order, OnFinishOrder)
        console.log(result)
