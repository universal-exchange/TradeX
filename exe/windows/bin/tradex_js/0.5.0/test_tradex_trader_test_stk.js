...
let func_stock_t_single_finish = 1200 // 单个证券委托成交 // 仅供测试交易
...
async function FinishOrder(module, order, callback) {
    ...
    g_event_call_finish = new AbortController()
    g_caller_dict.set(g_caller_id, new caller_item(g_caller_id))
    let result = JSON.parse(module.DirectCall(g_caller_id, func_stock_t_single_finish, 0, order.ToJson(), callback)) // 异步
    console.log(result['return_code'], result['return_info'], result['caller_id'])
    if(result['return_code'] !== 0) {
        return false
    }
...
async function Test_TradeX_Trader_Test_STK() {
    let kernel = new cyberx.Kernel(new syscfg.SysCfg()) // 全局唯一
    let module = new cyberx.Create('tradex_trader_test_stk') // 全局唯一
    //let module_01 = new cyberx.Create('tradex_trader_test_stk') // 重复创建会报异常
    //let module_01 = new cyberx.GetCreate('tradex_trader_test_stk') // 可以获取已创建的实例
    ...
    let deploy = new deploy_test_stk()
